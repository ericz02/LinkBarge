...
const registerUser = async (email, password, name, slug) => {
  ...
  const authReponse = supabase.auth.signUp({
    email, password
  })
  ...
  if (authResponse.data.user) {
    const addMetaResponse = await supabase
      .from("profile")
      .insert([{user_id: addMetaResponse.data.user.id, name, slug}])
...
export { registerUser, getLinksLinks, getCurrentUser, getSocialLinks }